...
export const Login = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            addToast(err.response?.data?.message || 'Giriş yapılamadı', 'error');
        }
    };
    ...
    return (
        <div className="min-h-screen flex items-center justify-center relative overflow-hidden bg-slate-50 pt-32 pb-20">
            {/* Decorative blobs */}
            <div className="absolute top-20 left-20 w-96 h-96 bg-indigo-200 rounded-full blur-3xl opacity-30 pointer-events-none" />
            <div className="absolute bottom-20 right-20 w-80 h-80 bg-cyan-200 rounded-full blur-3xl opacity-30 pointer-events-none" />

            <div className="w-full max-w-md bg-white p-10 rounded-3xl shadow-soft relative z-10 mx-4 border border-slate-100">
                <h2 className="text-4xl font-extrabold text-center mb-3 text-slate-800 tracking-tight">Tekrar Hoşgeldin</h2>
                <p className="text-slate-500 text-center mb-8">Hesabınıza erişmek için giriş yapın</p>
                ...
                <p className="mt-8 text-center text-slate-500 text-sm">
                    Hesabın yok mu? <Link to="/register" className="text-indigo-600 hover:text-indigo-700 font-bold hover:underline transition-colors">Ücretsiz kayıt ol</Link>
                </p>
            </div>
        </div>
    );
